 ...
 *
 * <p>
 * Responsável por criar, buscar, listar, atualizar e cancelar OS, garantindo
 * que todas as
 * regras do fluxo da oficina sejam respeitadas, como validação de
...
public class OrdemServicoService {
     ...
     * @throws BusinessException se cliente estiver inativo ou veículo não pertencer
     *                           ao cliente
     */
    @Transactional
    ...
    public Page<OrdemServicoResponseDTO> listar(Pageable pageable) {
        ...
        Sort sort = JpaSort.unsafe(
                "case " +
                        " when status = 'EM_ANDAMENTO' then 0 " +
                        " when status = 'ABERTA' then 1 " +
                        " when status = 'CONCLUIDA' then 2 " +
                        " when status = 'CANCELADA' then 3 " +
                        " else 9 end")
                .and(Sort.by("dataAbertura").ascending())
    ...
    public OrdemServicoResponseDTO atualizar(Long id, OrdemServicoUpdateDTO dto) {
        ...
        if (dto.status() != null) {
            ...
            // regra: não pode cancelar concluída (mantém tua regra do deletar)
            if (novoStatus == StatusOrdemServico.CANCELADA) {
                if (statusAtual == StatusOrdemServico.CONCLUIDA) {
                    throw new BusinessException("Não é possível cancelar uma OS concluída.");
                }
            ...
            if (novoStatus == StatusOrdemServico.CONCLUIDA) {
                ...
                boolean temItens = !itemRepository.findAllByOrdem_IdAndAtivoTrue(ordem.getId()).isEmpty();
                if (!temItens)
                ...
                // reabrir / voltar andamento / aberta
