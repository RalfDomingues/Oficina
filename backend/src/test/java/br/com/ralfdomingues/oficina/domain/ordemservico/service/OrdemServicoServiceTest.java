...
class OrdemServicoServiceTest {
    ...
    void listar_sucesso() {
        ...
        Pageable pageableIn = PageRequest.of(0, 10);
        Page<OrdemServico> page = new PageImpl<>(List.of(os));

        when(ordemRepo.findAll(any(Pageable.class))).thenReturn(page);

        var resultado = service.listar(pageableIn);

        assertEquals(1, resultado.getTotalElements());
        assertEquals(1L, resultado.getContent().get(0).id());

        ArgumentCaptor<Pageable> captor = ArgumentCaptor.forClass(Pageable.class);
        verify(ordemRepo).findAll(captor.capture());
    ...
    void listar_vazio() {
        ...
        when(ordemRepo.findAll(any(Pageable.class))).thenReturn(Page.empty());
